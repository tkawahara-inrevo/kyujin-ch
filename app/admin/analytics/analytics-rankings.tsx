  ...
  return (
    <>
      ...
      <div className="mt-8">
        ...
        <div className="mt-3 rounded-[12px] bg-white shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
          <div className="md:hidden">
            <div className="grid grid-cols-[36px_minmax(0,1fr)_84px] gap-2 border-b border-[#f0f0f0] px-4 py-3 text-[11px] font-semibold text-[#888]">
              <span>#</span>
            ...
            <div>
              {topCompanies.map((company) => (
                <button
                  ...
                  className="grid w-full grid-cols-[36px_minmax(0,1fr)_84px] items-center gap-2 border-b border-[#f8f8f8] px-4 py-3 text-left transition hover:bg-[#fafafa]"
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-[11px] text-[#999]">求人数 {company.jobsCount}</p>
                  </div>
            ...
            <table className="w-full text-left text-[13px]">
              <thead>
                <tr className="border-b border-[#f0f0f0] text-[#888]">
                  ...
                  <th className="px-5 py-3 font-semibold">総売上</th>
                </tr>
              </thead>
              <tbody>
                {topCompanies.map((company) => (
                  <tr key={company.id} className="border-b border-[#f8f8f8] hover:bg-[#fafafa]">
                    ...
                    <td className="px-5 py-3 font-medium text-[#10b981]">
                      ¥{company.totalCharge.toLocaleString()}
                    </td>
                  </tr>
                ))}
        ...
        <div className="fixed inset-0 z-50 flex items-end bg-black/40 md:hidden">
          ...
          <div className="relative w-full rounded-t-[24px] bg-white px-5 pb-6 pt-4 shadow-[0_-8px_24px_rgba(0,0,0,0.14)]">
            ...
            <div className="mt-5 grid grid-cols-3 gap-3">
              <Metric label="PV" value={selectedJob.viewCount.toLocaleString()} />
              <Metric label="応募数" value={String(selectedJob.applicationsCount)} />
              <Metric label="CVR" value={selectedJob.cvrLabel} />
            </div>
          </div>
        </div>
      )}
      ...
      {selectedCompany && (
        <div className="fixed inset-0 z-50 flex items-end bg-black/40 md:hidden">
          <button type="button" aria-label="close" onClick={() => setSelectedCompany(null)} className="absolute inset-0" />
          <div className="relative w-full rounded-t-[24px] bg-white px-5 pb-6 pt-4 shadow-[0_-8px_24px_rgba(0,0,0,0.14)]">
            <div className="mx-auto h-1.5 w-12 rounded-full bg-[#d1d5db]" />
            <div className="mt-4 flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[12px] font-semibold text-[#888]">#{selectedCompany.rank}</p>
                <h3 className="mt-1 truncate text-[16px] font-bold text-[#1e3a5f]">{selectedCompany.name}</h3>
              </div>
              <button type="button" onClick={() => setSelectedCompany(null)} className="rounded-full bg-[#f3f4f6] px-3 py-1.5 text-[12px] font-bold text-[#666]">
                閉じる
              </button>
            </div>
            <div className="mt-5 grid grid-cols-2 gap-3">
              <Metric label="求人数" value={String(selectedCompany.jobsCount)} />
              <Metric label="総売上" value={`¥${selectedCompany.totalCharge.toLocaleString()}`} />
            </div>
          </div>
        </div>
      )}
