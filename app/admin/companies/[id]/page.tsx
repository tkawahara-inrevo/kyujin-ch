...
export default async function AdminCompanyDetailPage({
  ...
  const company = await prisma.company.findUnique({
    ...
    include: {
      ...
      _count: { select: { invalidRequests: true, monthlyBillings: true } },
    },
  });
  ...
  return (
    <div className="p-6 lg:p-10">
      ...
      <div className="mt-4 flex items-center justify-between">
        <h1 className="text-[24px] font-bold text-[#1e293b]">{company.name}</h1>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="rounded-[12px] bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
          ...
          <dl className="space-y-3 text-[14px]">
            ...
            <InfoRow label="説明" value={company.description || "未設定"} />
            <InfoRow label="登録日" value={company.createdAt.toLocaleDateString("ja-JP")} />
          </dl>
        </div>

        <div className="rounded-[12px] bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
          <h2 className="mb-4 text-[16px] font-bold text-[#333]">担当者情報</h2>
          <dl className="space-y-3 text-[14px]">
            <InfoRow label="氏名" value={company.companyUser?.name || "未設定"} />
          ...
          <table className="w-full text-left text-[13px]">
            ...
            <tbody>
              {company.jobs.length === 0 ? (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-[#aaa]">求人がありません</td></tr>
              ) : (
