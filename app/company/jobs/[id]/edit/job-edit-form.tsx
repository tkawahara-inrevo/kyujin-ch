...
export function JobEditForm({
  ...
  const [benefitNote, setBenefitNote] = useState(job.benefitNote ?? "");
  ...
  useEffect(() => { markDirty(); }, [
    ...
    smokingNote, trainingInfo, selectedBenefits, benefitNote, selectedTags, selectionProcess,
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (description.length > 1000) {
      showError("仕事内容は1000文字以内で入力してください");
      return;
    }
    ...
    if ((smokingPolicyIndoor === "その他" || smokingPolicyOutdoor === "その他") && !smokingNote.trim()) {
      showError("受動喫煙対策で「その他」を選択した場合は特記事項を入力してください");
      return;
    }
        ...
        >
          ...
          <Section title="仕事内容" comment={rc("section-description", "description", "requirements")}>
            ...
            <Field label="求人タグ">
              ...
              {selectedTags.filter((t) => !ALL_TAG_OPTIONS.includes(t)).length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {selectedTags.filter((t) => !ALL_TAG_OPTIONS.includes(t)).map((t) => (
                    <span key={t} className="inline-flex items-center gap-1 rounded-full border border-[#1d63e3] bg-[#eef2ff] px-3 py-1 text-[13px] text-[#1d63e3]">
                      {t}
                      <button type="button" onClick={() => { setSelectedTags(selectedTags.filter((x) => x !== t)); markDirty(); }} className="ml-0.5 text-[#1d63e3] hover:text-[#eb0937]">×</button>
                    </span>
                  ))}
                </div>
              )}
          ...
          <Section title="福利厚生" comment={rc("section-benefits", "benefits")}>
            <Field label="福利厚生" required>
              ...
              <div className="mt-3">
                ...
                <textarea
                  value={benefitNote}
                  onChange={(e) => { setBenefitNote(e.target.value); markDirty(); }}
                  rows={2}
                  maxLength={500}
                  className={textareaCls}
          ...
          <Section title="受動喫煙対策" comment={rc("section-smoking", "smoking")}>
            ...
            {(() => {
              const noteRequired = smokingPolicyIndoor === "その他" || smokingPolicyOutdoor === "その他";
              return (
                <Field label="受動喫煙対策・特記事項" required={noteRequired}>
                  {noteRequired && (
          ...
          <div className="flex w-full max-w-[640px] flex-col rounded-[16px] bg-white shadow-[0_24px_80px_rgba(15,23,42,0.28)]" style={{ maxHeight: "85vh" }}>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <div>
                ...
                <textarea
                  value={benefitNote}
                  onChange={(event) => { setBenefitNote(event.target.value); markDirty(); }}
                  className={textareaCls}
                  rows={2}
                  placeholder="例：ランチ補助あり、書籍購入補助（年間3万円まで）など"
                />
