...
|---|---|
| `login` | `AuthViewModel` |
| `sign_up` | `RegisterViewModel` |
| `select_content` (job) | `JobDetailViewModel.init` |
| `favorite_add/remove` | `JobDetailViewModel.toggleFavorite` |
| `apply_job` (source=detail) | `ApplyViewModel.submit` |
| `apply_job` (source=swipe) | `SwipeViewModel.apply` |
| `swipe_card` | `SwipeViewModel.reject/apply` |
| `search` | `SearchViewModel.search` |
| `send_message` | `ThreadDetailViewModel.send` |
| `screen_view` | `MainActivity` (ナビゲーション一括) |

## screen_view (画面ビュー) 一覧

`MainActivity` の `MainShell` および `AppRoot` で `currentBackStackEntryAsState` を監視し、ルート文字列を `screenNameFor()` でマッピングして送信。
